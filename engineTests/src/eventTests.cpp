...
TEST(Events, ResizeConstructor) {
	...
	Engine::EventTypes type = e.getEventTypes();
	Engine::EventTypes staticType = e.getStaticType();
...
TEST(Events, CloseConstructor) {
	...
	EXPECT_TRUE(e.isInCategory(Engine::EventCategory::EventCategoryWindow));
	EXPECT_EQ(type, staticType);
	EXPECT_EQ(type, Engine::EventTypes::WindowClose);
}
